      ...
      try {
        ...
        if (skills && skills.length > 0) {
          for (const skill of skills) {
            await connection.execute("INSERT INTO skills (personal_info_id, name, proficiency) VALUES (?, ?, ?)", [
              personalInfoId,
              skill.name,
        ...
        if (categories && categories.length > 0) {
          for (const categoryId of categories) {
            await connection.execute("INSERT INTO cv_categories (cv_id, category_id) VALUES (?, ?)", [cvId, categoryId])
          }
        }
        ...
        if (personalInfo.address) doc.text(personalInfo.address, { align: "center" })
        ...
        if (education && education.length > 0) {
          ...
          education.forEach((edu) => {
            doc.fontSize(10).text(`${edu.degree} in ${edu.field_of_study || "N/A"}`)
        ...
        if (experience && experience.length > 0) {
          ...
          experience.forEach((exp) => {
            ...
            if (exp.start_date) {
              doc.text(`${exp.start_date} - ${exp.is_current ? "Present" : exp.end_date || "N/A"}`)
            }
        ...
        if (skills && skills.length > 0) {
          ...
          const skillsText = skills.map((s) => `${s.name} (${s.proficiency || "intermediate"})`).join(", ")
        ...
        await connection.execute("UPDATE cvs SET pdf_url = ? WHERE id = ?", [`/uploads/${pdfFileName}`, cvId])
...
router.get("/:id", verifyToken, param("id").isInt().withMessage("CV ID must be an integer"), async (req, res) => {
  // Check for validation errors
  const errors = validationResult(req)
  if (!errors.isEmpty()) {
    return res.status(400).json({
  ...
  try {
    ...
    // Get personal info
    const personalInfos = await query(`SELECT * FROM personal_info WHERE id = ?`, [cv.personal_info_id])

    cv.personalInfo = personalInfos[0]

    // Get education
    cv.education = await query(`SELECT * FROM education WHERE personal_info_id = ?`, [cv.personal_info_id])

    // Get experience
    cv.experience = await query(`SELECT * FROM experience WHERE personal_info_id = ?`, [cv.personal_info_id])

    // Get skills
    cv.skills = await query(`SELECT * FROM skills WHERE personal_info_id = ?`, [cv.personal_info_id])

    // Get projects
    cv.projects = await query(`SELECT * FROM projects WHERE personal_info_id = ?`, [cv.personal_info_id])
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (updateFields.length > 0) {
        ...
        await query(`UPDATE cvs SET ${updateFields.join(", ")} WHERE id = ?`, updateValues)
      }
      ...
      if (categories !== undefined) {
        ...
        if (categories.length > 0) {
          for (const categoryId of categories) {
            await query("INSERT INTO cv_categories (cv_id, category_id) VALUES (?, ?)", [cvId, categoryId])
          }
        }
      }
